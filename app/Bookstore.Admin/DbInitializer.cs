...
namespace AdminSite
{
    public static class DbInitializer
    {
        public static void Initialize(ApplicationDbContext context, string bucketName, string cdnDomain, string path)
        {
            ...
            var types = new BookType[] {
                new BookType{TypeName = "Hardcover" },
                new BookType{TypeName = "Trade Paperback"},
                new BookType{TypeName = "Mass-market paperback"}
            };
            foreach (BookType t in types)
            {
                context.Type.Add(t);
            }
            context.SaveChanges();

            BookType hardcoverType = context.Type.First(a => a.TypeName == "Hardcover");
